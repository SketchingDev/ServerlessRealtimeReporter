...
export const waitForProcessInAppSync = async (client: AWSAppSyncClient<any>, predicate: ProcessPredicate): Promise<Process | undefined> => {
...
export const hasTaskId = (expectedTaskId: string): ProcessPredicate =>
  ({tasks}) => (tasks.some(t => t.id === expectedTaskId));

export const and = (...predicates: ProcessPredicate[]): ProcessPredicate => process => predicates.every(predicate => predicate(process));
